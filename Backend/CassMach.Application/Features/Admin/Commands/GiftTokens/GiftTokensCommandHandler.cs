...
{
    public class GiftTokensCommandHandler : IRequestHandler<GiftTokensCommand, Result<TokenBalanceDto>>
    {
        private readonly ITokenService _tokenService;
        private readonly IUnitOfWork _unitOfWork;
        private readonly IMapper _mapper;

        public GiftTokensCommandHandler(ITokenService tokenService, IUnitOfWork unitOfWork, IMapper mapper)
        {
        ...
        public async Task<Result<TokenBalanceDto>> Handle(GiftTokensCommand request, CancellationToken cancellationToken)
        {
            var user = await _unitOfWork.Users.GetByIdAsync(request.UserId);
            if (user == null)
                return Result<TokenBalanceDto>.Failure(Error.Failure(ErrorCode.NotFound, "User not found"));

            await _tokenService.GiftTokens(request.UserId, request.CreditAmount, request.Description, request.AdminUserId);
